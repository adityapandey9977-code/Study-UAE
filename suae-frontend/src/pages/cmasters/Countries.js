...
export default function Countries(){
    ...
    const list=(p, ps)=>{
        ...
        util.showLoader();
        CmasterService.countries(sdataRef.current).then(({data})=>{
    ...
    const deleteRecord=(id)=>{
        ...
        confirm({
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteCountry(id).then(({data})=>{
    ...
    useEffect(()=>{
        list();
        CmasterService.allCurrencies().then(res=>setCurrencies(res.data.result.data));
                        ...
                        <table className="table table-bordered table-md table-striped table-hover m-0">
                            <thead className="thead-light text-uppercase table-text-vmid">
                                <tr>
                                    ...
                                    <th>Name</th>
                                    <th className="w70">Code</th>
                                    <th className="w70">Code3</th>
                                    <th className="w90">ISD Code</th>
                                    <th className="w150">Currency Code</th>
                                    <th className="w150">Currency Name</th>
...
const AddForm=forwardRef((props, ref)=>{
    let { callback, pageno, currencies } = props;
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveCountry(data).then(({data}) => {
    ...
    return (
        <Modal
            title={`${data.id ? 'Edit' : 'Add'} Country`}
            visible={showModal}
            okText="Save"
            onOk={handleOk}
            ...
            <form onSubmit={e=> {e.preventDefault(); handleOk()}} autoComplete="off" spellCheck="false">
                <div className="row mingap">
                    ...
                    <div className="col-md-12 form-group">
                        ...
                        <div>
                            <AntdSelect options={currencies} value={data.currency_id} onChange={v=>{handleChange(v, 'currency_id')}} />
                        </div>
                    </div>
                    <div className="col-md-12 form-group">
                        <label className="req">Code</label>
                        <Input value={data.code || ''} onChange={e=>handleChange(e.target.value.toUpperCase(), 'code')} />
                    </div>
                    <div className="col-md-12 form-group">
                        <label className="req">Code3</label>
                        <Input value={data.code3 || ''} onChange={e=>handleChange(e.target.value.toUpperCase(), 'code3')} />
                    </div>
                    <div className="col-md-12 form-group">
                        <label className="req">ISD Code</label>
                        <Input value={data.isd_code || ''} onChange={e=>handleChange(e.target.value.toUpperCase(), 'isd_code')} />
                    </div>
                    <div className="col-md-12 form-group">
