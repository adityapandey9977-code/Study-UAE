...
export default function CourseTypes(){
    ...
    const list=(p, ps)=>{
        ...
        util.showLoader();
        CmasterService.courseTypes(sdataRef.current).then(({data})=>{
    ...
    const deleteRecord=(id)=>{
        ...
        confirm({
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteCourseType(id).then(({data})=>{
...
const AddForm=forwardRef((props, ref)=>{
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveCourseType(data).then(({data}) => {
    ...
    return (
        <Modal
            title={`${data.id ? 'Edit' : 'Add'} course type`}
            visible={showModal}
            okText="Save"
            onOk={handleOk}
