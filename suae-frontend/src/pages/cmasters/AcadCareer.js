...
export default function AcadCareer(){
    ...
    const list=(p, ps)=>{
        ...
        util.showLoader();
        CmasterService.acadCareers(sdataRef.current).then(({data})=>{
    ...
    const deleteRecord=(id)=>{
        message.destroy();
        confirm({
            title: `Do you Want to delete this academic career?`,
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteAcadCareer(id).then(({data})=>{
...
const AddForm=forwardRef((props, ref)=>{
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveAcadCareer(data).then(({data}) => {
    ...
    return (
        <Modal
            title={`${data.id ? 'Edit' : 'Add'} Academic Career`}
            visible={showModal}
            okText="Save"
            onOk={handleOk}
