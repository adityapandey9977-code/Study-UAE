...
export default function Specialization(props) {
    ...
    const sdataRef=useRef({p:1, ps:50, course_id:props.course_id});
    const formRef = useRef();
    const list = (p, ps) => {
        sdataRef.current.p = p || 1;
        sdataRef.current.ps = ps || sdataRef.current.ps;
        util.showLoader();
        CmasterService.specializations(sdataRef.current).then(({ data }) => {
    ...
    const deleteRecord = (id) => {
        ...
        confirm({
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteSpecialization(id).then(({ data }) => {
    ...
    useEffect(()=>{
        list();
        CmasterService.allCourses({status:1}).then(({ data })=>setCourses(data.result.data));
            ...
            <div className="">
                ...
                <div className="d-flex tbl-search-head">
                    <div className="my-auto">
                        <SearchForm dataRef={sdataRef} onSearch={list} {...{courses}} />
                    </div>
                        ...
                        <table className="table table-bordered table-md table-striped table-hover m-0">
                            <thead className="thead-light text-uppercase table-text-vmid">
                                <tr>
                                    ...
                                    <th className="w200">Course</th>
                                    {/* <th className="w150">ERP Code</th> */}
...
const SearchForm=(props)=>{
    let {dataRef, onSearch, courses}=props;
    ...
    useEffect(() => {
        setData({...data, course_id:dataRef.current.course_id});
    }, [dataRef.current.course_id]);
    ...
    return (
        <form onSubmit={e => e.preventDefault()} autoComplete="off" spellCheck="false">
            ...
            <div className="d-flex cmasters-sf">
                ...
                <div className="mx-2 w350">
                    <AntdSelect options={courses} allowClear showSearch value={data.course_id} onChange={v => { handleChange(v, 'course_id') }} placeholder="Course" />
                </div>
                <div>
...
const AddForm = forwardRef((props, ref) => {
    let { callback, pageno, courses } = props;
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveSpecialization(data).then(({ data }) => {
    ...
    return (
        <Modal
            title={`${data.id ? 'Edit' : 'Add'} Specialization`}
            visible={showModal}
            okText="Save"
            onOk={handleOk}
            ...
            <form onSubmit={e => { e.preventDefault(); handleOk() }} autoComplete="off" spellCheck="false">
                <div className="row mingap">
                    <div className="col-md-12 form-group">
                        <label className="req">Specialization Name</label>
                        <Input value={data.name || ''} onChange={e => handleChange(e.target.value, 'name')} />
                    </div>
                    <div className="col-md-12 form-group">
                        <label className="req">Course</label>
                        <AntdSelect showSearch options={courses} value={data.course_id} onChange={v => { handleChange(v, 'course_id') }} />
                    </div>
                    {/* <div className="col-md-12 form-group">
