...
export default function IssuesCats(){
    ...
    const list=(p, ps)=>{
        ...
        util.showLoader();
        CmasterService.issuesCats(sdataRef.current).then(({data})=>{
    ...
    const deleteRecord=(id)=>{
        ...
        confirm({
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteIssuesCat(id).then(({data})=>{
...
const AddForm=forwardRef((props, ref)=>{
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveIssuesCat(data).then(({data}) => {
    ...
    return (
        <Modal
            title={`${data.id ? 'Edit' : 'Add'} Issues Categories`}
            visible={showModal}
            okText="Save"
            onOk={handleOk}
