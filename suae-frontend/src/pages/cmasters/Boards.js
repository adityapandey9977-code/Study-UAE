...
export default function Board(props) {
    ...
    const list=(p, ps)=>{
        ...
        util.showLoader();
        CmasterService.boards(props.type, sdataRef.current).then(({data})=>{
    ...
    const deleteRecord=(id)=>{
        message.destroy();
        confirm({
            title: `Do you Want to delete this ${props.type}?`,
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteBoard(id).then(({data}) => {
                        ...
                        <table className="table table-bordered table-md table-striped table-hover m-0">
                            <thead className="thead-light text-uppercase table-text-vmid">
                                <tr>
                                    ...
                                    <th>Name</th>
                                    {props.type==='Board' && <th className="w190">Board Type</th>}
                                    {/* <th className="w150">Display Order</th> */}
                            ...
                            <tbody className="table-text-top">
                                {result.data.map((v, i) => (
                                    <tr key={i}>
                                        ...
                                        {props.type==='Board' && 
                                            <td>
                                                {v.board_type?.split(',').sort().map((board, y)=>{
                                                    //let color = board === '10th' ? 'green' : (board === '12th' ? 'purple' : 'gold')
                                                    return board && <Tag key={y} color={"default"}>{board}</Tag>;
                                                })}
                                            </td>
                                        }
...
const AddForm = forwardRef((props, ref) => {
    let {callback, pageno, type} = props;
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveBoard(data).then(({data}) => {
    ...
    useImperativeHandle(ref, ()=>({
        openForm(dtl) {
            setData(dtl? {...dtl, board_type:dtl.board_type.split(',')}:{status:'1', type, board_type:[]});
            setShowModal(true);
        }
    }));
        ...
        >
            <form onSubmit={e=>{e.preventDefault(); handleOk()}} autoComplete="off" spellCheck="false">
                <div className="row mingap">
                    ...
                    {type==='Board' &&
                        <div className="col-md-12 form-group">
                            <label className="req">Board Type</label>
                            <div>
                                <AntdSelect
                                    mode="multiple"
                                    options={['10th', '12th', 'Diploma']}
                                    value={data.board_type}
                                    onChange={(v)=>{
                                        handleChange(v, 'board_type') 
                                    }}
                                />
                            </div>
                        </div>
