...
export default function Genders(){
    ...
    const list=(p, ps)=>{
        ...
        util.showLoader();
        CmasterService.genders(sdataRef.current).then(({data})=>{
    ...
    const deleteRecord=(id)=>{
        message.destroy();
        confirm({
            title: `Do you Want to delete this gender?`,
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteGender(id).then(({data})=>{
...
const AddForm=forwardRef((props, ref)=>{
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveGender(data).then(({data}) => {
