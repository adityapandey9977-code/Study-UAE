...
export default function Disciplines(){
    ...
    const list=(p, ps)=>{
        ...
        util.showLoader();
        CmasterService.disciplines(sdataRef.current).then(({data})=>{
    ...
    const deleteRecord=(id)=>{
        ...
        confirm({
            ...
            onOk() {
                util.showLoader();
                CmasterService.deleteDiscipline(id).then(({data})=>{
...
const AddForm=forwardRef((props, ref)=>{
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        CmasterService.saveDiscipline(data).then(({data}) => {
    ...
    return (
        <Modal
            title={`${data.id ? 'Edit' : 'Add'} Discipline`}
            visible={showModal}
            okText="Save"
            onOk={handleOk}
