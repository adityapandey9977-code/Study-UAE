...
export default function Notifications() {
    ...
    const sdataRef = useRef({ p: 1, ps: util.isStudent() === 1 ? 500 : 50 });
    ...
    const deleteRecord = (id) => {
        ...
        confirm({
            ...
            onOk() {
                ...
                StudentService.deleteNotification(id).then(({ data }) => {
    ...
    const view = (id, title, description) => {
        ...
        if (util.isStudent() === 1) {
            StudentService.markNotificationRead(id)
                .then(() => {
                    setResult(prev => {
                        const rows = (prev?.data || []).map(r => (String(r.id) === String(id) ? { ...r, isread: 1 } : r));
    ...
    const autoReload = () => {
        isAutoRefresh.current = setInterval(() => {
            if (sdataRef.current.p === 1 && util.isStudent() && window.location.href.indexOf("localhost") < 0) {
    ...
    return (
        <div className="page-content">
            <div className="page-head-gradient">
                <div>
                    <h2>
                        <i className="fa fa-bell"></i> {util.isStudent() !== 1 ? 'Student ' : ''}Notifications
                    </h2>
                </div>
                {util.isStudent() === 1 &&
                        ...
                        <div>
                            ...
                            <div className="position-relative">
                                <Spin spinning={isTableLoading}>
                                    <div className="border cscroll" style={{ height: wh - 230 }}>
                                        <List
                                            size="small"
                                            ...
                                            renderItem={(item, i) => (
                                                <List.Item>
                                                    <div className="cpointer" onClick={() => view(item.id, item.title, item.description)}>
                                                        <div className={item.isread ? "" : "bold600"}>{i + 1}. {item.title}</div>
                                                        <div className="text-secondary fs11">Sent on {util.getDate(item.created, 'DD MMM YYYY @ hh:mm A')}</div>
                                                        <div className="ellipsis note-text pt5 w100">
                                                            {item.description}
                                                        </div>
                                                    </div>
                                                </List.Item>
...
const SearchForm = (props) => {
    ...
    return (
        <form onSubmit={e => e.preventDefault()} autoComplete="off" spellCheck="false">
            <style>{`.notif-search .ant-input::placeholder { color: #000000 !important; font-weight: 700 !important; font-size: 14px !important; opacity: 1 !important; } .notif-search .ant-select-selection__placeholder { color: #000000 !important; font-weight: 700 !important; font-size: 14px !important; opacity: 1 !important; }`}</style>
            <div className="d-flex notif-search" style={{ gap: 8 }}>
                <div>
...
const AddForm = (props) => {
    ...
    const handleOk = () => {
        message.destroy();
        util.showLoader();
        StudentService.saveNotification(data).then(({ data }) => {
